...
<cffunction name="retrieveSequence" hint="Retrives the current ID for the sequence" access="private" returntype="numeric" output="false">
	...
	<cfquery name="qSelectSequence" datasource="#getDataSource().getName()#" username="#getDataSource().getUsername()#" password="#getDataSource().getPassword()#">
		...
		<cfcatch type="database">
			<!--- findif transfer sequence table not being there is the issue --->

			<!--- special case for oracle --->
			<cfif getDataSource().getDatabaseType() eq "oracle" and FindNoCase("ORA-01003", cfcatch.detail)>
				<cfreturn retrieveSequence(arguments.object, arguments.runOnce, false)>
			</cfif>

			<cfif NOT arguments.runOnce AND (FindNoCase(static.SEQUENCE_TABLE, cfcatch.detail) OR FindNoCase(static.SEQUENCE_TABLE, cfcatch.sql))>
				<cfscript>
					createSequenceTable();

					return retrieveSequence(arguments.object, true);
				</cfscript>
			<cfelse>
				<cfrethrow>
			</cfif>
