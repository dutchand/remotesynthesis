...
<cffunction name="selectStatement" hint="returns a query from a TQL Select statement" access="public" returntype="query" output="false">
	...
	<cfscript>
		var ast = getTQLParser().selectStatement(arguments.query.getTQL());
		var buffer = createObject("java", "java.lang.StringBuffer").init();
		var evaluation = arrayNew(1);
		var data = StructNew();

		if(arguments.query.getCacheEvaluation() AND getEvaluationCache().has(arguments.query.getTQL()))
		{
			evaluation = getEvaluationCache().get(arguments.query.getTQL());
		...
		{
			getEvaluationCache().add(arguments.query.getTQL(), evaluation);
		}
...
<cffunction name="evaluateSelectStatement" hint="Evaluates all sorts of select statements" access="public" returntype="array" output="false">
	...
	<cfscript>
		...
		if(arguments.tree.getType() eq getTQLParser().getNodeType("FROM")) //starts with 'from'
		{
			...
			evaluation = getFrom().evaluateFromAST(arguments.tree,
													aliasMap,
			...
			evaluation = evaluateTopLevelAST(arguments.tree, aliasMap, arguments.evaluation, arguments.buffer, arguments.aliasColumns, arguments.distinctMode);
		}
		else if(arguments.tree.getType() eq getTQLParser().getNodeType("SELECT"))
		{
			aliasMap = evaluateAliasMapFromAST(arguments.tree);
			evaluation = getSelectColumn().evaluateSelectAST(arguments.tree, aliasMap, arguments.evaluation, arguments.buffer, arguments.aliasColumns, arguments.distinctMode);
			evaluation = evaluateTopLevelAST(arguments.tree, aliasMap, arguments.evaluation, arguments.buffer, arguments.aliasColumns, arguments.distinctMode);
		}
		...
		<cfloop condition="#iterator.hasNext()#">
			...
			<cfif structKeyExists(block, "mappedParam")>
				...
				<cfswitch expression="#param.type#">
					<cfcase value="date">
						<cfqueryparam value="#value#" cfsqltype="cf_sql_date" list="#param.list#" null="#param.null#">
					</cfcase>
					<cfcase value="numeric">
						<cfqueryparam value="#value#" cfsqltype="cf_sql_float" list="#param.list#" null="#param.null#">
					</cfcase>
					<cfcase value="boolean">
						<cfqueryparam value="#value#" cfsqltype="cf_sql_bit" list="#param.list#" null="#param.null#">
					</cfcase>
					<cfdefaultcase>
						<cfqueryparam value="#value#" cfsqltype="cf_sql_varchar" list="#param.list#" null="#param.null#">
					</cfdefaultcase>
				</cfswitch>
			</cfif>
		</cfloop>
