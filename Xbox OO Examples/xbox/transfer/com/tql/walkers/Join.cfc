...
<cffunction name="evaluateJoinAST" hint="evaluates a join AST" access="public" returntype="array" output="false">
	...
	<cfscript>
		...
		{
			evaluateInterimAutoJoin(state.object, state.alias, arguments.buffer, arguments.pastClassIdentifierNodes);
		}
		else if(state.context eq "composite")
		{
			evaluateInterimCompositeJoin(arguments.tree, state.object, state.alias, arguments.aliasMap, arguments.buffer, arguments.pastClassIdentifierNodes);
		}

		//draw the join, based on the join state
		if(state.innerJoin)
		{
			arguments.buffer.append(" left ");
		}
		else
		{
...
<cffunction name="evaluateInterimAutoJoin" hint="evaluates any joining that must be done before joining the actual Object table" access="private" returntype="void" output="false">
	...
	<cfscript>
		...
		while(iterator.hasNext())
		{
			...
			resolveManyToManyInterimAutoJoinSQL(pastObject, pastAlias, arguments.object, arguments.alias, arguments.buffer);
		}
	</cfscript>
</cffunction>
...
<cffunction name="evaluateAutoJoin" hint="evaluates auto joining for a particlar node" access="private" returntype="array" output="false">
	...
	<cfscript>
		...
		while(iterator.hasNext())
		{
			...
			resolveAutoJoinSQL(pastObject, pastAlias, arguments.object, arguments.alias, arguments.buffer, buildRelationShipCollection(object, pastObject), on);
			on = false;
		}
...
<cffunction name="evaluateInterimCompositeJoin" hint="evaluate a on composite join" access="private" returntype="struct" output="false">
	...
	<cfscript>
		...
		{
			child = localTree.getChild(JavaCast("int", counter));
			arguments = evaluateInterimCompositeJoin(child, arguments.object, arguments.alias, arguments.aliasMap, arguments.buffer, arguments.pastClassIdentifierNodes);
		}
...
<cffunction name="resolveAutoJoinSQL" hint="resolves the joins found in the relationship collection" access="private" returntype="void" output="false">
	<cfargument name="objectFrom" hint="The object to go from" type="transfer.com.object.Object" required="Yes">
...
<cffunction name="buildManyToManyInterimJoinSQL" hint="Builds the from SQL for the intermim Many to Many table" access="private" returntype="void" output="false">
	...
	<cfscript>
		...
		{
			arguments.buffer.append(" left join ");
