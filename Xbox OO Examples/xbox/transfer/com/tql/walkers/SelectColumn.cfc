...
<cffunction name="evaluateSelectColumnsFromAST" hint="builds a list of select columns from the fromAST" access="public" returntype="struct" output="false">
	...
	<cfscript>
		...
		if(currentTree.getType() eq getTQLParser().getNodeType("CLASS_IDENTIFIER"))
		{
			...
			appendSelectColumnToBuffer(table, object.getPrimaryKey(), buffer, arguments.isFirstColumn, arguments.aliasColumns, arguments.usedColumnAliases);
			arguments.isFirstColumn = false;
		}
...
evaluateSelectColumnPrimaryKey(arguments.className, aliasMap, arguments.buffer, arguments.aliasColumns, arguments.distinctMode);
